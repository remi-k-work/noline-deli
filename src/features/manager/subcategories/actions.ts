...
import { createSubCategory, deleteSubCategory, updateSubCategory } from "@/features/manager/categories/db";
...
export const newSubCategory2 = actionClient
  ...
  .action(async ({ parsedInput }): Promise<SubCategoryFormActionResult> => {
    ...
    try {
      ...
      await createSubCategory((await getCreatedByUser()) ?? (await setCreatedByUser()), categoryId!, name);
    } catch (error) {
