...
export default function Entry({ product, createdByUser }: EntryProps) {
  ...
  return (
    ...
    >
      <TableCell>
        <article className={styles["entry-image"]}>
          <Link
            href={PathFinder.toProductEdit(id)}
            className={cn(styles["entry-image__link"], "transition-transform delay-150 duration-700 ease-in-out hover:translate-x-2")}
          >
            <Image
          ...
          <ProductInfoTrigger product={product} className={cn(styles["entry-image__info"], "z-10")} />
        </article>
      </TableCell>
      <TableCell>
      ...
      <TableCell>
        <Actions productId={id} productName={name} productImageUrl={imageUrl} productPrice={price} />
      </TableCell>
    </TableRow>
  );
}
